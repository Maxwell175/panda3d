...
void print_usage(const char *prog) {
  std::cout <<
    ...
    "  --model PATH       Model to clone.  If unset, uses the built-in\n"
    "                     planet_sphere.egg.pz from the solar-system sample.\n"
    "  --threading-model M\n"
...
int main(int argc, char **argv) {
  ...
  if (model_path.empty()) {
    model_path = "samples/solar-system/models/planet_sphere.egg.pz";
  }
  NodePath model = window->load_model(framework.get_models(), Filename(model_path));
  if (model.is_empty()) {
    std::cerr << "error: could not load model: " << model_path << "\n"
              << "       supply --model PATH (working dir: try the panda3d source root)\n";
    return 1;
  }
  model.detach_node();  // workers attach copies, not the prototype
